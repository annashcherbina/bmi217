...
def get_predictions_sequential(hdf5_source,batch_size,individual_task_output_shape,model):
    num_generated=0
    ...
    while num_generated < total_entries:
        ...
        start_index=num_generated
        end_index=min([total_entries,start_index+batch_size])
        x_batch=hdf5_source[start_index:end_index]
        predictions_batch=model.predict(x_batch)
        #add the predictions to the dictionary
...
def parse_args():
    ...
    parser.add_argument('--sequential',help='use this flag if your model is a sequential model',action="store_true")
...
def main():
    ...
    if args.sequential==True: 
        inputs=data['X']['sequence']
        outputs=data['Y']['output']
    else:
        inputs=data['X']
        outputs=data['Y'] 
